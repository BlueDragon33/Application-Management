...
export async function recordManagedAppSession(input: {
  ...
  await database.batch([
    ...
    database.prepare(`INSERT INTO managed_app_sessions
      (session_id, app_id, device_id, device_code, expires_at)
      VALUES (?, ?, ?, ?, ?)`)
      .bind(id, input.appId, input.deviceId, input.deviceCode, input.expiresAt),
...
export async function listManagedAppSessions(appId: "hoa-nhap-nga", limit = 40) {
  await ensureTable();
  const database = await getControlDatabase();
  const rows = await database.prepare(`SELECT session_id, app_id, device_id, device_code, issued_at, expires_at,
    last_seen_at, revoked_at, revoke_reason
    FROM managed_app_sessions WHERE app_id = ? ORDER BY issued_at DESC LIMIT ?`)
