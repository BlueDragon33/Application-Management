...
export const applicationRegistry: readonly ApplicationConfig[] = [
  ...
  },
  {
    ...
    contractNote: "NC03 Control Center v0.6.2 đã map HAR2 firmware 8.00.42: % pin, kết nối, 4G/5G/nhà mạng và chất lượng sóng luôn hiển thị; telemetry 10 giây cập nhật tại chỗ và giữ last-known-good khi poll lỗi tạm thời, có nhãn Đang kết nối lại/Dữ liệu gần nhất. UI và Local Bridge dùng chung policy RFC1918-only. Read-only mở rộng cho Mobile Data, SIM PIN, Cloud SIM auto-switch, network settings, 4 Wi-Fi AP, USB/Cradle, IP Passthrough, security/filter/DMZ, NTP, power/display, data usage, FOTA và count-only DHCP/port/filter inventory. App Management mở runtime/contract NC03 local; modem credential không đi qua control-plane và write vẫn khóa tới khi WRITE VERIFIED.",
    ...
    deviceExperiences: standardDeviceExperiences,
    capabilities: ["Mở NC03 Control Center", "Live telemetry 10 giây không rerender trang", "Pin % chính xác luôn hiển thị", "Kết nối + sóng + 4G/5G", "Mobile Data / SIM PIN / Cloud SIM read-only", "Firmware 8.00.42 HAR2 read profile", "4 Wi-Fi AP", "USB/Bridge/Security/NTP/Power/FOTA read-only", "Count-only DHCP/Port/Packet-filter inventory", "Thiết bị kết nối", "RFC1918-only Local Bridge", "Security gate", "Offline/PWA self-refresh"],
