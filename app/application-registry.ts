...
export const applicationRegistry: readonly ApplicationConfig[] = [
  ...
  },
  {
    ...
    contractNote: "NC03 Control Center v0.5.1 đã map HAR thật firmware 8.00.42, đọc được status/pin chính xác/Wi-Fi/client và có local runtime riêng. Application Management mở website NC03 qua local launcher 127.0.0.1:3006; credential/session modem vẫn ở thiết bị người dùng. Login request và write operation vẫn tiếp tục fail-closed cho tới khi được xác minh.",
    ...
    deviceExperiences: standardDeviceExperiences,
    capabilities: ["Truy cập NC03 Control Center local", "Trạng thái build/release", "API discovery progress", "Firmware 8.00.42 capability matrix", "Pin % chính xác", "Wi-Fi & client read path", "Security gate", "Offline/PWA gate"],
