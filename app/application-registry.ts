...
export const applicationRegistry: readonly ApplicationConfig[] = [
  ...
  },
  {
    ...
    contractNote: "NC03 Control Center v0.6.3 đã hoàn tất read-path HAR2 firmware 8.00.42: % pin, kết nối, 4G/5G/nhà mạng và chất lượng sóng luôn hiển thị; telemetry 10 giây cập nhật tại chỗ, giữ last-known-good, hiển thị giờ cập nhật gần nhất và không tự hiện xanh lại khi chuyển tab. Advanced snapshot có LIVE READ/LAST GOOD/retry riêng. UI và Local Bridge dùng chung policy RFC1918-only; Remember Password vẫn khóa tới khi AUTH VERIFIED. App Management mở runtime/contract NC03 local; modem credential không đi qua control-plane và write vẫn khóa tới khi WRITE VERIFIED.",
    ...
    deviceExperiences: standardDeviceExperiences,
    capabilities: ["Mở NC03 Control Center", "Live telemetry 10 giây không rerender trang", "Last-known-good + timestamp", "Pin % chính xác luôn hiển thị", "Kết nối + sóng + 4G/5G", "Advanced snapshot freshness + retry", "Mobile Data / SIM PIN / Cloud SIM read-only", "Firmware 8.00.42 HAR2 read profile", "4 Wi-Fi AP", "USB/Bridge/Security/NTP/Power/FOTA read-only", "Count-only DHCP/Port/Packet-filter inventory", "Thiết bị kết nối", "RFC1918-only Local Bridge", "Security gate", "Offline/PWA self-refresh"],
