    ...
    <section className="modernWorkspace">
      ...
      <div className="modernPage">
        ...
        <section className="modernBoard">
          ...
          <section className="modernPanel modernAppsPanel"><PanelTitle icon="◇" title="Ứng dụng đang quản lý" onClick={() => go("applications")}/><div className="modernAppsTable"><div className="modernAppsHead"><span>Ứng dụng</span><span>Nhóm nghiệp vụ</span><span>Việc chờ xử lý</span><span>Thiết bị online</span><span>Trạng thái</span><span>Thao tác</span></div>{primaryApps.map((app) => { const summary = summaryMap.get(app.id); const state = connection(summary, app); const pending = summary?.pendingCount ?? devices.filter((item) => item.appId === app.id && item.status === "pending").length; return <div className="modernAppsRow" key={app.id}><AppCell appId={app.id} name={app.shortName}/><span>{app.id === "health-care" ? "Y tế" : app.id === "ru-life" ? "Nga" : app.id === "boi-ech" ? "Học tập" : "Học thuật"}</span><strong>{pending}</strong><strong>{summary?.onlineCount ?? 0}</strong><b data-state={state}><i/>{connectionText(state)}</b><button onClick={() => window.location.assign(app.href)}>Vào quản trị</button></div>; })}</div></section>
