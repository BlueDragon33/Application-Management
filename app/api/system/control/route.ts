...
async function auditRows() {
  const database = await getControlDatabase();
  const result = await database.prepare(
    "SELECT id, actor, action, target, detail_json, created_at FROM control_audit_log ORDER BY id DESC LIMIT 200",
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "bootstrap") {
      return json({
        actor,
        controlDevices: actor.role === "owner" ? await controlDevices() : [],
        auditLog: ["publisher", "owner"].includes(actor.role) ? await auditRows() : [],
      });
    }

    if (action === "manage-control-device") {
      if (actor.role !== "owner") throw new ControlAccessError("Chỉ chủ hệ thống được quản lý tài khoản và thiết bị quản trị.", 403, "OWNER_REQUIRED");
      ...
      if (targetId === actor.deviceId || await isOwnerEmail(target.email)) {
      ...
      if (operation === "approve") {
        ...
        await database.batch([
          ...
          ).bind(target.email, displayName || null, role, actor.email),
        ...
        await audit(actor.email, "control_device_approved", targetId, { role });
        ...
        await audit(actor.email, "control_device_blocked", targetId);
        ...
        await audit(actor.email, "control_member_deactivated", target.email, { targetDeviceId: targetId });
        ...
        await audit(actor.email, "control_member_deleted", target.email, { targetDeviceId: targetId });
      } else {
        throw new ControlAccessError("Thao tác thiết bị quản trị không hợp lệ.", 400, "INVALID_CONTROL_OPERATION");
      }

      return json({ ok: true, controlDevices: await controlDevices() });
    }

    throw new ControlAccessError("Thao tác Hệ thống không hợp lệ.", 400, "INVALID_SYSTEM_ACTION");
  } catch (error) {
