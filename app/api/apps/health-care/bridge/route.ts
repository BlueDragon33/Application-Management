...
import { HealthBridgeError, issueHealthBrowserBridge } from "../../../../health-care.server";
...
export async function POST(request: Request) {
  try {
    ...
    if (action !== "bootstrap") {
      return json({ error: "Thao tác bridge Sức khỏe Y tế không hợp lệ.", code: "INVALID_HEALTH_BRIDGE_ACTION" }, 400);
    }

    return json({
      actor,
      application: "health-care",
      bridge: await issueHealthBrowserBridge(actor.email, actor.role, actor.deviceId),
    });
  } catch (error) {
