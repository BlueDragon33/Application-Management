...
import { issueRuLifeBrowserBridge, RuLifeBridgeError } from "../../../../ru-life.server";
...
export async function POST(request: Request) {
  try {
    ...
    if (action !== "bootstrap") {
      return json({ error: "Thao tác bridge Hòa nhập Nga không hợp lệ.", code: "INVALID_RU_LIFE_BRIDGE_ACTION" }, 400);
    }

    return json({
      actor,
      application: "ru-life",
      bridge: await issueRuLifeBrowserBridge(actor.email, actor.role, actor.deviceId),
    });
  } catch (error) {
