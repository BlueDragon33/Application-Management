...
export default function BoiEchControlCenter({ user }: { user: { displayName: string; email: string } }) {
  ...
  const [tab, setTab] = useState<"devices" | "ai" | "content">("devices");
  ...
  return (
    ...
    >
      <aside className="control-sidebar">
        <div className="control-brand"><div className="brand-seal">BE</div><div><span>Ứng dụng độc lập</span><strong>Quản trị Bơi ếch</strong></div></div>
        <nav>
      ...
      <section className="control-main">
        <header className="topbar"><div><span>Bơi ếch · Client admin</span><h1>{tab === "devices" ? "Thiết bị học tập" : tab === "ai" ? "Điều hành AI" : "Duyệt chỉnh sửa"}</h1></div><div className="topbar-actions"><span className={`sync-state ${boiOnline && online ? "" : "sync-warning"}`}><i /> {!online ? "Máy quản trị đang offline" : boiOnline ? "Trạng thái tự động · 60 giây/lần" : "Đang giữ dữ liệu gần nhất"}</span><button className="button appearance-control" onClick={() => setAppearanceOpen(true)} aria-expanded={appearanceOpen} aria-controls="admin-appearance-panel"><b>Aa</b> Giao diện</button><button className="button" onClick={() => void refresh(credential, access, { discoverNew: true })} disabled={checking || !online}>{checking ? "Đang cập nhật…" : "Cập nhật thiết bị"}</button></div></header>
