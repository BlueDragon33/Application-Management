  ...
  return <main className={styles.workspaceShell}>
    ...
    <section className={styles.workspaceMain}>
      ...
      {view === "overview" ? <>
        ...
        <section className={styles.clientPanel}><div className={styles.panelHeader}><div><span>CONTROL SCOPE</span><h2>Quản trị tối thiểu, không gom dữ liệu</h2></div><p>Application Management chỉ cần biết thiết bị, quyền, phiên bản dịch vụ và audit metadata an toàn khi các API tương ứng tồn tại.</p></div><div className={styles.contractSummary}><div><span>Runtime</span><strong>GrowUP sở hữu</strong></div><div><span>Child data</span><strong>Không đưa lên control-plane</strong></div><div><span>Health data</span><strong>Không đưa lên control-plane</strong></div><div><span>Admin operations</span><strong>{siteState.remoteAdminReady ? "Có backend thật" : "Chờ backend riêng"}</strong></div></div></section>
        <section className={styles.boundaryNotice}><span>!</span><div><strong>Direct web launch không đồng nghĩa với quyền đọc dữ liệu trẻ em.</strong><p>Không bật các nút quản trị giả. Trung tâm chỉ mở Site GrowUP đã xác minh; registry, audit từ xa và kiểm duyệt cấu hình vẫn bị khóa cho tới khi backend GU- thật tồn tại.</p></div></section>
      </> : null}
