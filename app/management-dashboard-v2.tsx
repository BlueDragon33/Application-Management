...
export default function ManagementDashboardV2({ user, authMode }: {
  ...
  return <main className="amv2-shell" data-font-scale={fontScale}>
    ...
    {autoPolicyOpen ? <AutomaticDevicePolicies key={operations?.generatedAt ?? "loading"} apps={activeApps} settings={operations?.settings} busy={actionBusy === "auto-policy"} close={() => setAutoPolicyOpen(false)} save={(selection) => void saveAutomation(selection)}/> : null}
