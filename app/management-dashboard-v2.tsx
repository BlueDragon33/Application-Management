...
export default function ManagementDashboardV2({ user }: { user: { displayName: string; email: string } }) {
  ...
  const notificationCount = pendingDevices.length + distinctWorkItems.length;
  const users = useMemo(() => {
  ...
  async function dismissNotifications() {
    ...
    try {
      ...
      setNotice("Đã xóa hết thông báo hiển thị. Dữ liệu và yêu cầu thiết bị gốc vẫn được giữ nguyên.");
  ...
  return <main className={styles.shell} style={appearanceStyle}>
    ...
    <section className={styles.workspace}>
      ...
      </div><footer className={styles.statusFooter}><span>Quản trị Ứng dụng Ver2</span><span>{clock ? new Intl.DateTimeFormat("vi-VN", { dateStyle: "short", timeStyle: "medium" }).format(clock) : ""}</span><span>{syncWarnings ? `${syncWarnings} ứng dụng cần kiểm tra` : "Hệ thống đồng bộ ổn định"}</span></footer>
    </section>
  </main>;
}
